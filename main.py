...
def generate_secure_password(length=12, use_uppercase=True, use_digits=True, use_special=True, exclude_ambiguous=False):
    """
    ...
    if exclude_ambiguous:
        ...
        special_characters = ''.join([c for c in special_characters if c not in ambiguous_chars])
    ...
    password = [
        ...
        random.choice(special_characters) if use_special else random.choice(lowercase)
    ]
...
if __name__ == "__main__":
    try:
        ...
        exclude_ambiguous = input("Exclude ambiguous characters (0, O, 1, l)? (yes/no): ").strip().lower() == 'yes'

        password = generate_secure_password(length, use_uppercase, use_digits, use_special, exclude_ambiguous)
        print(f"Generated secure password: {password}")

        save = input("Do you want to save the password to a file? (yes/no): ").strip().lower()
        if save == 'yes':
